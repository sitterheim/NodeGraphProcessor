...
namespace NodeGraphProcessor.Editor
{
	...
	public class BaseGraphView : GraphView, IDisposable
	{
		...
		protected override bool canCopySelection => selection.Any(e => e is BaseNodeView || e is GroupView);

		protected override bool canCutSelection => selection.Any(e => e is BaseNodeView || e is GroupView);
		...
		public virtual EdgeView CreateEdgeView() => new();
		...
		public override List<Port> GetCompatiblePorts(Port startPort, NodeAdapter nodeAdapter)
		{
			...
			compatiblePorts.AddRange(ports.ToList()
				.Where(p =>
				{
		...
		protected virtual void BuildGroupContextualMenu(ContextualMenuPopulateEvent evt, int menuPosition = -1)
		{
			...
			evt.menu.InsertAction(menuPosition, "Create Group", e => AddSelectionsToGroup(AddGroup(new Group("Create Group", position))),
...
#if UNITY_2020_1_OR_NEWER
			var position = (evt.currentTarget as VisualElement).ChangeCoordinatesTo(contentViewContainer, evt.localMousePosition);
			evt.menu.InsertAction(menuPosition, "Create Sticky Note", e => AddStickyNote(new StickyNote("Create Note", position)),
				DropdownMenuAction.AlwaysEnabled);
#endif
		}
		...
		/// </summary>
		/// <param name="evt"></param>
		protected virtual void BuildSelectAssetContextualMenu(ContextualMenuPopulateEvent evt) => evt.menu.AppendAction("Select Asset",
			e => EditorGUIUtility.PingObject(graph), DropdownMenuAction.AlwaysEnabled);

		/// <summary>
		/// Add the Save Asset entry to the context menu
		/// </summary>
		/// <param name="evt"></param>
		protected virtual void BuildSaveAssetContextualMenu(ContextualMenuPopulateEvent evt) => evt.menu.AppendAction("Save Asset", e =>
		{
		...
		private void MouseUpCallback(MouseUpEvent e) => schedule.Execute(() =>
			{
		...
		private void UpdateSerializedProperties() => serializedGraph = new SerializedObject(graph);
		...
		public void ToggleView<T>() where T : PinnedElementView => ToggleView(typeof(T));
		...
		public DropdownMenuAction.Status GetPinnedElementStatus<T>() where T : PinnedElementView => GetPinnedElementStatus(typeof(T));
