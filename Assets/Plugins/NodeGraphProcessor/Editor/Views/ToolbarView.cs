...
namespace NodeGraphProcessor.Editor
{
	public class ToolbarView : VisualElement
	{
		...
		protected void HideButton(string name) => leftButtonDatas.Concat(rightButtonDatas)
			.All(b =>
			{
		...
		/// </summary>
		/// <param name="name">Display name of the button</param>
		protected void ShowButton(string name) => leftButtonDatas.Concat(rightButtonDatas)
			.All(b =>
			{
				if (b?.content?.text == name)
					b.visible = true;
