...
namespace NodeGraphProcessor
{
	public class BaseEdgeDragHelper : EdgeDragHelper
	{
		...
		internal Vector2 GetEffectivePanSpeed(Vector2 mousePos)
		{
			...
			{
				effectiveSpeed.x = ((mousePos.x - (graphView.contentContainer.layout.width - k_PanAreaWidth)) / k_PanAreaWidth + 0.5f) *
				                   k_PanSpeed;
			}
			...
			{
				effectiveSpeed.y = ((mousePos.y - (graphView.contentContainer.layout.height - k_PanAreaWidth)) / k_PanAreaWidth + 0.5f) *
				                   k_PanSpeed;
			}
		...
		protected virtual EdgeView CreateEdgeView() => new();
