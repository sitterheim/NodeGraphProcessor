...
namespace NodeGraphProcessor.Editor
{
	...
	{
		private static readonly Dictionary<Type, Type> fieldDrawers = new();

		private static readonly MethodInfo createFieldMethod =
			typeof(FieldFactory).GetMethod("CreateFieldSpecific", BindingFlags.Static | BindingFlags.Public);

		public static INotifyValueChanged<T> CreateField<T>(T value, string label = null) =>
			CreateField(value != null ? value.GetType() : typeof(T), label) as INotifyValueChanged<T>;
			...
			else
			{
				try
				{
					field = Activator.CreateInstance(drawerType,
						...
						new[] { label, Type.Missing }, CultureInfo.CurrentCulture);
				}
				catch
				{
					field = Activator.CreateInstance(drawerType,
		...
		{
			var fieldDrawer = CreateField(value, label);

			if (fieldDrawer == null)
				return null;

			fieldDrawer.value = value;
			fieldDrawer.RegisterValueChangedCallback(e => { onValueChanged(e.newValue); });
		...
		public static VisualElement CreateField(Type fieldType, object value, Action<object> onValueChanged, string label)
		{
			...
			if (fieldType == typeof(LayerMask))
			{
				...
				layerField.RegisterValueChangedCallback(e => { onValueChanged(new LayerMask { value = e.newValue }); });
			...
			else
			{
				try
				{
					...
					try
					{
						field = createFieldSpecificMethod.Invoke(null, new[] { value, onValueChanged, label }) as VisualElement;
					}
					catch {}

					// handle the Object field case
					if (field == null && (value == null || value is Object))
					{
						createFieldSpecificMethod = createFieldMethod.MakeGenericMethod(typeof(Object));
						field = createFieldSpecificMethod.Invoke(null, new[] { value, onValueChanged, label }) as VisualElement;
						if (field is ObjectField objField)
						{
							objField.objectType = fieldType;
							objField.value = value as Object;
						}
					}
				}
				catch (Exception e)
		...
		private static void AddDrawer(Type fieldType, Type drawerType)
		{
