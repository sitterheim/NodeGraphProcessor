...
[Serializable] [NodeMenuItem("Conditional/ForLoop")]
public class ForLoopNode : ConditionalNode
{
	...
	public IEnumerable<ConditionalNode> GetExecutedNodesLoopBody() =>
		...
		outputPorts.FirstOrDefault(n => n.fieldName == nameof(loopBody))
			.GetEdges()
			.Select(e => e.inputNode as ConditionalNode);

	public IEnumerable<ConditionalNode> GetExecutedNodesLoopCompleted() =>
		// Return all the nodes connected to the executes port
		outputPorts.FirstOrDefault(n => n.fieldName == nameof(loopCompleted))
			.GetEdges()
			.Select(e => e.inputNode as ConditionalNode);
}